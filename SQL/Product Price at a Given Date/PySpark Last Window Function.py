...
spark = SparkSession.builder \
    .appName("Last Prices with last() Function") \
    .getOrCreate()
...
# Create a window specification for last value
window_spec = Window.partitionBy('product_id').orderBy(F.desc('change_date'))

# Use last() to get the last new_price per product_id
last_prices = filtered_df.select(
    'product_id',
    F.last('new_price', ignorenulls=True).over(window_spec).alias('new_price')
).distinct()
